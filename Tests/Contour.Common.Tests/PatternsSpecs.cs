...
namespace Contour.Common.Tests
{
    ...
    public class PatternsSpecs
    {
        ...
        /// <summary>
        /// “естова€ конфигураци€ дл€ проверки оператора подтверждени€ обработки сообщени€.
        /// </summary>
        [TestFixture]
        [Category("Unit")]
        ...
        {
            /// <summary>
            /// ѕровер€ет, что оператор подтверждает сообщение.
            /// </summary>
            [Test]
            public void should_accept_message()
            {
                ...
                result.Should().HaveCount(1, "—ообщение должно быть передано дальше.");
                deliveryMock.Verify(dm => dm.Accept(), "ƒолжна быть вызвана операци€ подтверждени€ обработки.");
            }
        }

        /// <summary>
        /// “естова€ конфигураци€ дл€ проверки оператора ответа на запрос с передачей сообщени€ дальше.
        /// </summary>
        [TestFixture]
        [Category("Unit")]
        ...
        {
            /// <summary>
            /// ѕровер€ет, что оператор ответа на запрос передает сообщение дальше.
            /// </summary>
            [Test]
            ...
            /// <summary>
            /// ѕровер€ет, что оператор отвечает на запрос переданным сообщением.
            /// </summary>
            [Test]
            public void should_reply_by_message()
            {
                ...
                deliveryMock.Verify(d => d.ReplyWith(It.IsAny<IMessage>()), "ƒолжна быть вызвана отправка ответного сообщени€.");
            }
        }

        /// <summary>
        /// “естова€ конфигураци€ дл€ проверки оператора перенаправлени€ сообщени€ динамическому списку получателей.
        /// </summary>
        [TestFixture]
        [Category("Unit")]
        ...
        {
            /// <summary>
            /// ѕровер€ет, что оператор возвращает список сообщений в соответствии со списком получателей.
            /// </summary>
            [Test]
            public void should_pass_through_message()
            {
                ...
                result.Should().HaveCount(labels.Length, " оличество сообщений должно соответствовать полученному списку получателей.");
                result.Should().Contain(m => m.Label.ToString() == "boo.r1", "ƒолжно быть сообщение c меткой boo.r1");
                result.Should().Contain(m => m.Label.ToString() == "boo.r2", "ƒолжно быть сообщение c меткой boo.r2");
                result.Should().Contain(m => m.Label.ToString() == "boo.r3", "ƒолжно быть сообщение c меткой boo.r3");
            }
        }

        /// <summary>
        /// “естова€ конфигураци€ дл€ проверки оператора инспеции сообщений в шине.
        /// </summary>
        [TestFixture]
        [Category("Unit")]
        ...
        {
            /// <summary>
            /// ѕровер€ет, что оператор возвращает исходное сообщение и его дубликат с другой меткой.
            /// </summary>
            [Test]
            public void should_pass_through_message()
            {
                ...
                result.Should().HaveCount(2, "ƒолжно быть исходное сообщение и его дубликат.");
                result.Should().Contain(m => m.Label.ToString() == "boo.r1", "ƒолжно быть сообщение c меткой boo.r1");
                result.Should().Contain(m => m.Label.ToString() == "boo", "ƒолжно быть сообщение c меткой boo");
            }
        }
    }
